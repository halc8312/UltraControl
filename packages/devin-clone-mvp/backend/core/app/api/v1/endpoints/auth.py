...
from app.models import User, Session, SubscriptionPlan
from app.schemas.auth import (
    UserSignUp, UserSignIn, Token, TokenRefresh,
    PasswordReset, PasswordResetConfirm, GuestUserCreate
)
...
def generate_guest_username() -> str:
    ...
    return f'guest_{random_suffix}'


def generate_guest_email() -> str:
    """Generate a random email for guest users"""
    random_suffix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
    return f'guest_{random_suffix}@guest.devinclone.app'
...
async def guest_login(
    *,
    ...
    """
    Create and authenticate a guest user
    """
    ...
    user = User(
        ...
        tokens_limit=1000,  # Lower limit for guest users
        tokens_used=0,
        tokens_reset_at=datetime.utcnow() + timedelta(days=1)  # Guest session expires in 1 day
    )
    ...
    session = Session(
        user_id=user.id,
        refresh_token=security.get_password_hash(refresh_token),
        user_agent="Guest Login",
